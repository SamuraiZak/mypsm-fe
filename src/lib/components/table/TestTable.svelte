<script lang="ts">
    ...
    export let nestedHeaderIndex: number | undefined = 7;
    export let nestedHeaderChild: number | undefined = 5;
    export let headRowTotal: number = 2;
    export let headerRowSize: number = 2;


    function getColSpan(index:number){
        if (index === nestedHeaderIndex){
            return nestedHeaderChild;
        } else {
            return 1;
        }
    }
    ...
    >
        ...
        <div class="flex flex-wrap items-center justify-start gap-2.5">
            ...
            <Button
                ...
                on:click={previous}><SvgChevronLeft size="14" /></Button
            >
            {#if currentPage > 3}
                <PaginationItem
            ...
            <Button
                ...
                class="h-[28px] rounded-[3px] border border-bdr-primary bg-bgr-primary px-2.5 text-sm text-txt-secondary hover:bg-bgr-tertiary focus:outline-none focus:ring-0"
                on:click={next}><SvgChevronRight size="14" /></Button
            >
            <Select
                placeholder=""
            ...
            >
                {#if updatedTableItems.length !== 0}
                    <TableHead>
                        ...
                        <!-- {console.log(Object.keys(updatedTableItems[0]).length)} -->
                        <!-- START OF HEADER NAMING -->
                        {#each Array.from({length: headRowTotal}) as _, tablerow}
                        <TableBodyRow>
                        {#each Object.entries(updatedTableItems[0]) as [key, value],i}
                            {#if key !== 'selected'}
                                {#if columnKeys.length > 0}
                                    {#if columnKeys.includes(key)}
                                        {#if typeof value !== 'object'}
                                            <th
                                                class="px-6 py-2 border text-center  cursor-pointer"
                                                on:click={() => sort(key)}
                                            >
                                                <div
                                                    class="flex flex-row items-center justify-center gap-1"
                                    ...
                                    {/if}
                                {:else if typeof value !== 'object'}
                                <!-- {console.log(Object.keys(updatedTableItems[0]).length)} == 9 -->
                                {console.log(key)}
                                <!-- TO DO: IF COLUMN WITH ROW SPAN OF 2 = GET OUT OF THE LOOP -->

                                    <!-- {#each key as controlColumn, controlIndex(controlIndex)}
                                        {#if }
                                    {/each} -->
                                    <th
                                        rowspan={getRowSpan(tablerow)}
                                        colspan={getColSpan(tablerow)}
                                        class="px-6 py-2 border text-center  cursor-pointer"
                                        on:click={() => sort(key)}
                                    >
                                        <div
