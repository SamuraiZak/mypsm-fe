...
export async function load({ params }) {
    ...
    let supplierDetailRequestBody: loanIdRequestDTO = {
        id: currentApplicationId
    }

    const supplierDetailResponse: CommonResponseDTO =
        await LoanServices.getSupplierDetails
            (supplierDetailRequestBody);

    if (supplierDetailResponse.status == 'success') {
        supplierDetail = supplierDetailResponse.data?.details as Supplier;
    }
    ...
    const firstScheduleDetailsForm = await superValidate(firstScheduleDetailResponse.data?.details as FirstSchedule, zod(
        ...
        const supplierDetailsForm = await superValidate(supplierDetailResponse.data?.details as Supplier, zod(
            _supplier))
            ;
