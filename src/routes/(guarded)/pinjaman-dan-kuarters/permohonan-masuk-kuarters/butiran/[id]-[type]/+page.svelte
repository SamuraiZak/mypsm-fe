<script lang="ts">
    ...
    const {
        ...
        async onSubmit() {
            // const readOnly =
            //     await _submitOutsiderApplicationForm($personalDetailForm);

            // if(readOnly.status == "success"){
            //     submitPersonalDetail = true;
            //     outsiderId
            // }
            _submitOutsiderApplicationForm($personalDetailForm).then((result) => {
                if(result.status == "success"){
                    submitPersonalDetail = true;
                    outsiderId = result.data?.details.outsiderId
                    console.log(outsiderId)
                }
            })
        },
    });
    ...
    if ($personalDetailForm.name !== '' && $personalDetailForm.name !== undefined) {
        submitPersonalDetail = true;
    }
...
>
    <Stepper>
        <StepperContent>
            <StepperContentHeader title="Jenis Pemohon"></StepperContentHeader>
            <StepperContentBody>
        ...
        <StepperContent>
            <StepperContentHeader title="Maklumat Pasangan">
                {#if !outsiderFamilyForm && data.currentRoleCode == UserRoleConstant.urusSetiaPeringkatNegeri.code}
                    <TextIconButton
                        label="Simpan"
                        icon="check"
                        form="outsiderFamily"
        ...
        <StepperContent>
            ...
            >
                {#if !outsiderService && data.currentRoleCode == UserRoleConstant.urusSetiaPeringkatNegeri.code}
                    <TextIconButton
                        label="Simpan"
                        icon="check"
                        form="outsiderServiceForm"
