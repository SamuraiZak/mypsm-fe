...
const generalDateSchema =  z.coerce
.date({
...
const minDateScheme = z.string().pipe( z.coerce.date() )


const generalSelectSchema = z.number({invalid_type_error: "Sila tetapkan pilihan anda.", required_error: "Sila tetapkan pilihan anda. ",description: "Sila tetapkan pilihan anda."}).refine((data) => data > 0, {message: "Sila tetapkan pilihan anda"});
...
export const _updateInterviewDetails = z.object({
    ...
    position: generalSelectSchema,
    interviewTime: GeneralTextSchema,
    state: generalSelectSchema,
    interviewVenue: GeneralTextSchema,
    meetingDate: generalDateSchema,
    interviewDate: generalDateSchema,
});
...
export const _promotionMeetingResultSchema = z.object({
    ...
    .string({ required_error: 'Medan ini latihan tidak boleh kosong.' })
    .min(1, {
        message: 'Sila keputusan mesyuarat .',
    })
    .max(4, {
        message: 'Medan ini tidak boleh melebihi 4 karakter.',
    }),
    actingPosition: generalSelectSchema,
    actingGred: generalSelectSchema,
});
...
export const _submitUpdateInterviewDetailsForm = async (formData: Object) => {
    ...
    if (!updateInterviewDetailsForm.valid) {
        getErrorToast();
        console.log(updateInterviewDetailsForm)
