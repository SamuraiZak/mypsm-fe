<script lang="ts">
    ...
    let isSupportedStateDirector: boolean =
        data.record.currentApplication.status ===
        'SOKONG - Pengarah Bahagian/Negeri';
    let isFreeIntegrityDirector: boolean =
        data.record.currentApplication.status === 'BEBAS - Pengarah Integriti';
    let isCertifiedCourseSecretary: boolean =
        data.record.currentApplication.status === 'SAH - Urus Setia Latihan';
...
>
    ...
    <StepperContent>
        ...
        <div class="flex w-full flex-col gap-2">
            <StepperContentBody>
                {#if data.record.currentApplication.status === 'Baru'}
                    ...
                    >
                        <label for="file-download" class="w-[220px]"
                            >Menunggu keputusan Pengarah Bahagian/Negeri...</label
                        >
                    </div>
                {:else if isSupportedStateDirector}
                    <div
                ...
                {:else if isFreeIntegrityDirector || isCertifiedCourseSecretary || isCompleted}
                    <div
                        class="h-fit w-full space-y-2.5 rounded-[3px] border p-2.5"
                    >
                        <div class="mb-5">
    ...
    <StepperContent>
        <StepperContentHeader
            title="Pengesahan Maklumat Oleh Urus Setia Latihan"
        >
            {#if isFreeIntegrityDirector}
                <TextIconButton primary label="Simpan" />
            {/if}
        </StepperContentHeader>
        <StepperContentBody>
            {#if isFreeIntegrityDirector || isCertifiedCourseSecretary || isCompleted}
                <LongTextField
                    disabled={isCompleted || isCertifiedCourseSecretary}
                    labelBlack={false}
                    name="meeting-remark"
                    label="Tindakan/Ulasan"
                    value={'semakan sah'}
            ...
            {:else}
                ...
                >
                    <label for="file-download" class="w-[220px]"
                        >Menunggu keputusan Peranan - Peranan Lain...</label
                    >
                </div>
            {/if}
        </StepperContentBody>
    ...
    <StepperContent>
        <StepperContentHeader title="Keputusan Permohonan Pembiayaan Pelajaran"
            >{#if isCertifiedCourseSecretary}
                <TextIconButton primary label="Simpan" />
            {/if}</StepperContentHeader
        >
        <StepperContentBody>
            {#if isCertifiedCourseSecretary || isCompleted}
                <div
                    class="mb-2.5 flex w-full flex-row items-center text-sm italic"
                >
                    <label for="success-application" class="w-[220px]"
            ...
            {:else}
                ...
                >
                    <label for="file-download" class="w-[220px]"
                        >Menunggu keputusan Urus Setia Latihan...</label
                    >
                </div>
            {/if}
        </StepperContentBody>
