...
export async function _submitForm(
    ...
    let form: LookupItemDTO = formData as LookupItemDTO;
    let result: CommonResponseDTO = CommonResponseConstant.httpError;

    if (isNew) {
        result = await LookupManagementServices.add(form, type);
    } else {
        result = await LookupManagementServices.edit(form, type);
    }
