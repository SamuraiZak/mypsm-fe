<script lang="ts">
    ...
    const { form, errors, enhance } = superForm(
        data.bantuanMengurusJenazahForm,
        {
            SPA: true,
            validators: _stepperBantuanMengurusJenazah,
            onSubmit() {
                _submitMengurusJenazahForm($form);
            },
            taintedMessage:
...
<section>
    <div>
        ...
        >
            ...
            >
                ...
                <DateSelector
                hasError={$errors.tarikhMeninggal ? true : false}
                name="tarikhMeninggal"
                handleDateChange
                bind:selectedDate={$form.tarikhMeninggal}
                label="Tarikh Meninggal"
                ></DateSelector>
                {#if $errors.tarikhMeninggal}
                <span
                    class="ml-[220px] font-sans text-sm italic text-system-danger"
                    >{$errors.tarikhMeninggal}</span
                >
            {/if}
                <TextField
                hasError={$errors.masaMeninggal ? true : false}
                name= "masaMeninggal"
                label="Masa Meninggal"
                 placeholder="-"
                bind:value={$form.masaMeninggal}>
            </TextField>
            {#if $errors.masaMeninggal}
            <span
                class="ml-[220px] font-sans text-sm italic text-system-danger"
                >{$errors.masaMeninggal[0]}</span
            >
        {/if}
            </div>
        </form>
    ...
    >
        ...
        <TextField label="Nama Waris" placeholder="-"></TextField>
        <TextField label="No. K/P" placeholder="-"></TextField>
        <TextField label="Hubungan" placeholder="-"></TextField>
    </div>
</section>
