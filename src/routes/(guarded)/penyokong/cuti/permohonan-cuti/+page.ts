...
export const load = async () => {
    ...
    const replacementLeaveHistoryList: LeaveHistoryListResponse[] = (
        leaveHistoryListResponse.data?.dataList as LeaveHistoryListResponse[]
    ).filter((data) => data.leaveType === 'Gantian');

    const untrackedLeaveHistoryList: LeaveHistoryListResponse[] = (
        leaveHistoryListResponse.data?.dataList as LeaveHistoryListResponse[]
    ).filter((data) => data.leaveType === 'Tanpa Rekod');

    return {
        replacementLeaveHistoryList,
        untrackedLeaveHistoryList,
        param,
    };
};
