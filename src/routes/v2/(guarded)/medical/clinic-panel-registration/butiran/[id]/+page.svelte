<script lang="ts">
    ...
    const {
        ...
        async onSubmit() {
            ...
            $clinicVerificationForm.isDraft = false;
...
<section class="flex w-full flex-col items-start justify-start">
    <ContentHeader title="Maklumat Klinik">
        <TextIconButton
            ...
            type="neutral"
            label="Tutup"
            onClick={() => goto('/v2/medical/clinic-panel-registration')}
        />
    </ContentHeader>
</section>
...
>
    <Stepper>
        <StepperContent>
            ...
            <StepperContentBody>
                <div class="flex w-full flex-col justify-start p-2 pb-10">
                ...
                <div class="flex w-full flex-col justify-start p-2 pb-10">
                    <div class="flex w-1/2 flex-col justify-start gap-2.5">
                        {#if (data.applicationDoc?.document?.length < 1 || data.applicationDoc?.document == null) && (data.currentRoleCode == UserRoleConstant.unitBahagian.code || data.currentRoleCode == UserRoleConstant.unitNegeri.code)}
                            <div
                                class="flex w-full flex-col justify-start gap-2 px-3 pb-5 text-sm text-ios-labelColors-secondaryLabel-light"
                            >
                                <span
                                    >Dokumen-dokumen yang perlu dimuat naik:</span
                                >
                                <span>1. Salinan Maklumat Klinik</span>
                                <span>2. Salinan Maklumat Bank Klinik</span>
                            </div>
                        ...
                        {:else}
                            <div
                                class="flex w-full flex-col justify-start gap-3 text-sm text-ios-labelColors-secondaryLabel-light"
                            >
                                <span
                                    >Dokumen-dokumen sokongan yang telah dimuat
                                    naik :</span
                                >
                                {#if data.applicationDoc?.document?.length > 0}
                                    {#each data.applicationDoc?.document as documents, i}
                                        <div
                                            class="flex w-full items-center gap-4 text-base text-ios-systemColors-systemGrey3-dark"
                                        >
                                            <span>{i + 1}.</span>
                                            <a
                            ...
                            <Alert color="blue">
                                <p>
                                    <span class="font-medium"
                                        >Tiada Maklumat!</span
                                    >
                                    Menunggu maklumat lantikan daripada Urus Setia
                                    Perubatan.
                                </p>
                            </Alert>
                        </div>
        ...
        <StepperContent>
            ...
            <StepperContentBody>
                <div class="flex w-full flex-col justify-start p-2 pb-10">
                    ...
                    {:else}
                        <form
                            class="flex w-1/2  flex-col justify-start gap-2.5 px-2 pb-10"
                            id="supporterApproverForm"
                            method="POST"
        ...
        {#if data.clinicSecretaryApproval.remark !== null}
            <StepperContent>
                <StepperContentHeader title="Keputusan Penyokong">
                    {#if !notEditingSupporterForm && data.currentRoleCode == UserRoleConstant.kakitangan.code}
                        <TextIconButton
                            icon="check"
                            type="primary"
                            label="Hantar"
                ...
                <StepperContentBody>
                    <div class="flex w-full flex-col justify-start p-2 pb-10">
                        {#if data.clinicSupporterApproval.remark == null && data.currentRoleCode !== UserRoleConstant.kakitangan.code}
                            <div class="flex w-1/2  flex-col">
                                <Alert color="blue">
                                    <p>
                                        <span class="font-medium"
            ...
            <StepperContent>
                <StepperContentHeader title="Keputusan Pelulus"
                    >{#if !notEditingApproverForm && data.currentRoleCode == UserRoleConstant.kakitangan.code}
                        <TextIconButton
                            icon="check"
                            type="primary"
                            label="Hantar"
                ...
                <StepperContentBody>
                    <div class="flex w-full flex-col justify-start p-2 pb-10">
                        {#if data.clinicApproverApproval.remark == null && data.currentRoleCode !== UserRoleConstant.kakitangan.code}
                            <div class="flex w-1/2 flex-col">
                                <Alert color="blue">
                                    <p>
                                        <span class="font-medium"
