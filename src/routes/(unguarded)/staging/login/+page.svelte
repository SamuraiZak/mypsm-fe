<script lang="ts">
    ...
    import SvgPeople from '$lib/assets/svg/SvgPeople.svelte';
</script>
...
>
    <div class="space-y-4 p-6 sm:p-8 md:space-y-6">
        ...
        <div class="flex w-full flex-col gap-2">
            <button
                on:click={() => goto('/staging/login/kakitangan')}
                type="button"
                class="bg h-20 rounded-md bg-ios-systemColors-quaternarySystemFill-light text-base hover:bg-ios-systemColors-quaternarySystemFill-dark"
            >
                Kakitangan LKIM
            </button>
            <button
                on:click={() => goto('/staging/login/calon')}
                type="button"
                class="bg h-20 rounded-md bg-ios-systemColors-quaternarySystemFill-light text-base hover:bg-ios-systemColors-quaternarySystemFill-dark"
            >
            ...
            <button
                on:click={() => goto('/staging/login/klinik-panel')}
                type="button"
                class="bg h-20 rounded-md bg-ios-systemColors-quaternarySystemFill-light text-base hover:bg-ios-systemColors-quaternarySystemFill-dark"
            >
                Klinik Panel
